...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    @Override
    @Transactional(readOnly = true)
    public List<EmployeeResponse> findByReportsTo(Integer reportsToId) {
        return employeeRepository.findByReportsToEmployeeId(reportsToId).stream().map(this::toResponse).toList();
    }
    ...
    private Employee toEntity(EmployeeRequest request, Employee employee) {
        ...
        employee.setReportsTo(request.getReportsToId() == null
                ...
                : employeeRepository.findById(request.getReportsToId())
                        .orElseThrow(() -> new ResourceNotFoundException("ReportsTo Employee", request.getReportsToId())));
